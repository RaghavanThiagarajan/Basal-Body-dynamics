...
// The matlab script "tend_time_dist_spatial_plot_all.m" gives "TrajecInitPos.csv" where all the initial BB position details are stored. In this script, we use 
// these positions to get the mean intensity values of actin from the Actin movie (Actin_gradient.tif), in the corresponding tend positions of BB trajectories. Unlike 
// "Actin_intensity_for_bb_tend_ROIs_Traj_after_filtering.ijm", all BB trajectories without any filtering are used for the analysis. 
// For more explanation on why filtering was done in "Actin_intensity_for_bb_tend_ROIs_Traj_after_filtering.ijm", look at the first section of "Gradients_BB_int_normalization_by_cortex_Traj_after_filtering.m" matlab script.
