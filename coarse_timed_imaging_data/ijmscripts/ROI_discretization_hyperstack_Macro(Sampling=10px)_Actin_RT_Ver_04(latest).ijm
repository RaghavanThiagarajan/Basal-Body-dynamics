...
// User Prompt
waitForUser("Open the files 'Actin_gradient.tif' and 'Actin_gradient_rawdata.tif' \n             from the folder: 'Input_Actin' and then click 'OK'");
...
setBatchMode(false);
selectWindow("ROI Manager"); roiManager("save", dir1 +"ROI_Backgroundintensity" + ".zip"); run("Close");
...
selectWindow("ROI Manager"); roiManager("save", dir0_0 +"ROI_gradient_of_actin_intensity" + ".zip"); run("Close");
setBatchMode(false);	
...
setBatchMode(false);	
selectWindow("ROI Manager"); roiManager("save", dir12 +"ROI_for_cortex" + ".zip"); run("Close");
...
roiManager("open", dir0_0 +"ROI_gradient_of_actin_intensity" + ".zip");
setBatchMode(true);
for (t=1; t<=totalframes; t++) {
	for (n=1; n<=totalslices; n++) {
