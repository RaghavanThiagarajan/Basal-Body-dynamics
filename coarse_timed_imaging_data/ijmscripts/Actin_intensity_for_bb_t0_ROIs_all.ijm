...
// In this script, we use the already obtained t0 positions / initial positions of all trajectories from the Matlab analysis to get the actin intensity in these positions. 
// The matlab script "t0_time_dist_spatial_plot_all.m" gives "TrajecInitPos.csv" where all the initial BB position details are stored. In this script, we use 
// these positions to get the mean intensity values of actin from the Actin movie (Actin_gradient.tif), in the corresponding t0 positions of BB trajectories. Unlike 
// "Actin_intensity_for_bb_t0_ROIs_Traj_after_filtering.ijm", all BB trajectories without any filtering are used for the analysis. 
// For more explanation on why filtering was done in "Actin_intensity_for_bb_t0_ROIs_Traj_after_filtering.ijm", look at the first section of "Gradients_BB_int_normalization_by_cortex_Traj_after_filtering.m" matlab script.
...
dir0 = dir_0 + "/bb_t0_ROIs_data/"; File.makeDirectory(dir0); 
...
open(dir_0+"/TrajecInitPos.csv");
Trajectory_elements = getValue("results.count"); 
Total_trajectories = Table.get("Trajectory", (Trajectory_elements-1)); 
Trajectory_no = newArray; Frame_t0 = newArray; xcoord = newArray; ycoord = newArray; MeanInt_BBposition = newArray; 
MeanInt_SurrBBposition = newArray; xcoord_surr = newArray; ycoord_surr = newArray; Frame_t0_surr = newArray; Trajectory_no_surr = newArray;
...
for (I = 0; I <= (Trajectory_elements-1); I++) {
	selectWindow("TrajecInitPos.csv");
	...
	for (f = 0; f <= totalframes-1; f++) {
		...
		Frame_t0[increment+f] = Fram;
		Trajectory_no[increment+f] = Current_trajectoryno;
	}
	increment = increment+totalframes;
}
selectWindow("Results"); close("Results");
selectWindow("ROI Manager"); roiManager("save", dir0 +"Actin_intensity_at_bb_t0_ROIs" + ".zip"); run("Close");
...
for (I = 0; I <= (Trajectory_elements-1); I++) {
	...
	for (f = 0; f <= totalframes-1; f++) {
		...
		Frame_t0_surr[increment+f] = Fram;
		Trajectory_no_surr[increment+f] = Current_trajectoryno;
	}
	increment = increment+totalframes;
}
selectWindow("Results"); close("Results");
selectWindow("ROI Manager"); roiManager("save", dir0 +"Actin_intensity_Surr_bb_t0_ROIs" + ".zip"); run("Close");
selectWindow("TrajecInitPos.csv"); run("Close");
// Use the command below for sanity checking / trouble shooting
//Table.showArrays("Actin_intensity_at_&_surr_bb_t0_ROIs", Trajectory_no, Trajectory_no_surr, Frame_t0, Frame_t0_surr, xcoord, xcoord_surr, ycoord, ycoord_surr, MeanInt_BBposition, MeanInt_SurrBBposition); 
Table.showArrays("Actin_intensity_at_&_surr_bb_t0_ROIs", Trajectory_no, Frame_t0, xcoord, ycoord, MeanInt_BBposition, MeanInt_SurrBBposition); 
selectWindow("Actin_intensity_at_&_surr_bb_t0_ROIs"); Table.save(dir0 + "ActInt_at&sur_BBt0_ROIs" + ".csv"); run("Close");
