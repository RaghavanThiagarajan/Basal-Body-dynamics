...
for ij = 1:TrajecendPos_sz
    ...
    inputlink = strcat('../Plots/tend_basalbody_position_', sprintf('%01d', traj_filter_duration), 'minlongTraj/bb_disappearance_input/bb_');
    bbimagesinp = strcat(inputlink, sprintf('%03d',framenobb), '.tif');
    savelink_tend = strcat('../Plots/tend_basalbody_position_', sprintf('%01d', traj_filter_duration), 'minlongTraj/bb_disappearance_plot/bb_');
...
savelink_tend = strcat('../Plots/tend_basalbody_position_', sprintf('%01d', traj_filter_duration), 'minlongTraj/');
...
% longer than "traj_filter_duration" (in no of frames) were selected.
% However here the nu_of_trajectories corresponds to only those trajectories that are longer than "traj_filter_duration" (in no of frames). This change is done because of the way the data is stored in those files from which the data is input.
...
frm_nmb_time_zeroremoved = frm_nmb_time_zeroremoved;
...
figure(1);
stem(frm_nmb_time_zeroremoved(1,(1:(end-2))), basalbody_count_modified_rate(1,(1:(end-1))));
...
BB_flux = basalbody_count_modified_rate ./ Area_tend_BB(1,(1:(end-1))); 

figure(1);
bbflux_fig = scatter(frm_nmb_time_modified(1,(1:(end-2))), BB_flux(1,(1:(end-1))), 'LineWidth',1); bbflux_fig.MarkerEdgeColor = 'k';
...
figure (3);
stem(Area_tend_BB(1,(1:(end-2))), basalbody_count_modified_rate(1,(1:(end-1))));
...
%%
disp('Run the macro "Actin_intensity_for_bb_tend_ROIs_Traj_after_filtering.ijm". Then press a key !') 
pause;

%% Getting the meanintensity of actin before and after basal body disappearance 

BBTend = importdata(strcat('../Plots/tend_basalbody_position_', sprintf('%01d', traj_filter_duration), 'minlongTraj/bb_tend_ROIs_data/ActInt_at&sur_BBtend_ROIs.csv'));
BBtend = BBTend.data;
clear BBTend
...
saveas(gcf, fullfile(montage_fig,'slide_15_1_entry_exit_actin_Int'), 'tif');
...
saveas(gcf, fullfile(montage_fig,'slide_15_2_BBrates'), 'tif');
...
saveas(gcf, fullfile(montage_fig,'slide_15_3_BBflux'), 'tif');

close all;
%%
save('workspace_with_tend_data');
