...
% This script is a continuation of the 'Gradients_BB_int_normalization_by_cortex_all.m' (characteristics of all trajectories without any filtering) script where we calculate the correlations between actin intensities & BB
...
correlation = '../Plots/ActinInt_at&around_basalbody_position_all/Apical_domain/Correlations';
...
%% Section 2
load('workspace_interim.mat', 'max_apical_area_idx');
...
% Also two important differences between the plots here and the already plotted ones in 'Gradients_BB_int_normalization_by_cortex_Traj_after_filtering.m' are: (1) here, the starting points of all trajectories are aligned - in other words, 
% the starting point of all trajectories start at row 1 (in reality they all start at different frames). Thats why the plots look different. If the intensities are seen w.r.to when they are starting during expansion, 
% the previous plots should be used, On the other hand, if the intensities of trajectories are investigated only for their intensity changes over time without any reference to expansion, then the plots here should be used.
% (2) here, only those trajectories that exist at the end (max area / final time point) are plotted to demonstrate that the final intensities (at max. area / final time point) are higher than the initial intensities. Rest of the 
% trajectories are left out because they are not present at the max. area time point.
...
%%
save('workspace_interim_all'); % This is to save all the variables in the workspace from "Gradients_BB_int_normalization_by_cortex_current" so that it can be used while getting the BB exit data. This is important because lot of variables in the below script is reused in the BB exit 
...
saveas(gcf, fullfile(montage_fig,'slide_8_all'), 'tif');
...
saveas(gcf, fullfile(montage_fig,'slide_9_1_all'), 'tif');
...
saveas(gcf, fullfile(montage_fig,'slide_9_2_all'), 'tif');
...
montage({fig6, fig18});
saveas(gcf, fullfile(montage_fig,'slide_10_all'), 'tif');
...
saveas(gcf, fullfile(montage_fig,'slide_11_all'), 'tif');
...
saveas(gcf, fullfile(montage_fig,'slide_12_all'), 'tif');
...
fig37 = fullfile(correlation, 'comparison_meanIntBBpos.tif');
fig38 = fullfile(correlation, 'comparison_totalIntBBpos.tif');
fig39 = fullfile(correlation, 'comparison_meanIntBBsurpos.tif');
fig40 = fullfile(correlation, 'comparison_totalIntBBsurpos.tif');
fig41 = fullfile(correlation, 'comparison_meanIntBBratio.tif');
fig42 = fullfile(correlation, 'comparison_totalIntBBratio.tif');

montage({fig25, fig26, fig27, fig28, fig29, fig30, fig31, fig32,fig33, fig34, fig35, fig36, fig37, fig38, fig39, fig40, fig41, fig42});
saveas(gcf, fullfile(montage_fig,'slide_13_all'), 'tif');
